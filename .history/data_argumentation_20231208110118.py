...
def data_argumentation():
    ...
    for i in range(pixels.shape[0]):
        ...
        Image.fromarray(x[0].squeeze()).convert('L').save(f"data/fer2013/images/aug_{data_df['emotion'][i]}_{data_df['Usage'][i]}_{i}.jpeg")
        if data_df['Usage'][i] == 'PublicTest':
            print(f"Skip the image {i} in PublicTest")
            pass
        else:
            j = 0
            for batch in datagen.flow(x, batch_size=1, save_to_dir='data/fer2013/images/', save_prefix=f"aug_{data_df['emotion'][i]}_{data_df['Usage'][i]}_{i}_{j+1}", save_format='jpeg'):
                j += 1
                if j >= 4:
                    break
        ...
        assert len(data_list) == 5 * (i + 1), f"The number of augmented images is {len(data_list)}, should be {5 * (i + 1)},\
            the image number is {len(image_files)}"
        for image_file in image_files:
