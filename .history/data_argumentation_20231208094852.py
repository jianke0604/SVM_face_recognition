...
def data_argumentation():
    ...
    for i in range(pixels.shape[0]):
        ...
        Image.fromarray(x[0].squeeze()).convert('L').save(f"data/fer2013/images/aug_{data_df['emotion'][i]}_{data_df['Usage'][i]}.jpeg")
        if data_df['Usage'][i] == 'PublicTest':
            pass
        else:
            j = 0
            for batch in datagen.flow(x, batch_size=1, save_to_dir='data/fer2013/images/', save_prefix=f"aug_{data_df['emotion'][i]}_{data['Usage'][i]}", save_format='jpeg'):
                j += 1
                if j > 4:
                    break
            assert j == 5, f"The number of augmented images is {j}"
